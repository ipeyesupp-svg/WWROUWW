...
class SnusbaseClient(BaseClient):
    ...
    def search(self, query: str, search_type: str) -> Dict[str, Any]:
        # search_type определяет endpoint и payload
...
class DragonAggregator:
    def __init__(self):
        ...
        # Маппинг типов поиска → список задач (функция, args, kwargs, alias)
        self.routes = {
            "username": [
    ...
    def _format_args(self, template_args: list, query: str) -> list:
        """Подставляет query в шаблоны аргументов"""
        out = []
    ...
    def _format_kwargs(self, template_kwargs: dict, query: str) -> dict:
        """Рекурсивно подставляет query в строки kwargs"""
        out = {}
...
def search(req: SearchRequest):
    """
    Единый endpoint поиска.

    **Доступные типы:**
    - `username` — Telegram username
    - `id` — Telegram numeric ID
...
def list_types():
    """Список доступных типов поиска и подключаемых API к каждому"""
    mapping = {}
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("dragon_aggregator:app", host="0.0.0.0", port=8000, reload=False)
