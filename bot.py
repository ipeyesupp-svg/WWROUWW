...
async def jitler_search(search_type: str, query: str, page: int = 1) -> Optional[Dict]:
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            if r and r.status_code == 200:
                return r.json()
            elif r and r.status_code in (400, 401, 403, 429):
                # Try next key on client errors (400, 401, 403, 429)
...
def _build_sections(labels, results) -> list:
    ...
    for label, data in zip(labels, results):
        ...
        if text and len(text.strip()) > 2:
            # Use actual label instead of Base #counter
...
def create_html_report(title, sections, report_type):
    ...
    for i, (section_title, section_data) in enumerate(sections):
        ...
        # Check if data is essentially empty (empty dict, empty list, or short string)
        data_str = str(section_data).strip()
        if isinstance(section_data, dict) and not section_data:
            continue
        if isinstance(section_data, list) and not section_data:
            continue
        if isinstance(section_data, str) and len(data_str) < 10:
            continue
...
def process_fanstat(message):
    ...
    def _do_search():
        try:
            ...
            # Jitler API integration for Telegram search
            try:
                jitler_result = run_async(jitler_search("telegram", query.replace("@", "")))
                if jitler_result:
                    # Try different response structures
                    data = None
                    if isinstance(jitler_result, dict):
                        if "response" in jitler_result:
                            response = jitler_result["response"]
                            if isinstance(response, dict):
                                ...
                                data = response
                        elif "result" in jitler_result and jitler_result["result"]:
                            if isinstance(jitler_result["result"], dict):
                                data = json.dumps(jitler_result["result"], indent=2, ensure_ascii=False)
                            elif isinstance(jitler_result["result"], str) and jitler_result["result"].strip():
                                data = jitler_result["result"]
                        elif jitler_result.get("data"):
                            data = json.dumps(jitler_result["data"], indent=2, ensure_ascii=False)
                        elif jitler_result.get("results"):
                            data = json.dumps(jitler_result["results"], indent=2, ensure_ascii=False)
                        else:
                            data = json.dumps(jitler_result, indent=2, ensure_ascii=False)
                    elif isinstance(jitler_result, str) and jitler_result.strip():
                        data = jitler_result
                ...
                print(f"Jitler telegram search error: {e}")
                pass
